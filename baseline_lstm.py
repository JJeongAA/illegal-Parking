...
# [설정] 결과 저장 폴더 지정
# ---------------------------------------------------------
SAVE_DIR = 'lstm_3channel_results'
...
class LSTMModel(nn.Module):
    ...
    def __init__(self, input_channels, hidden_size, num_layers, output_height, output_width, 
        ...
        # LSTM 입력 차원 계산 (32채널 * H * W를 flatten)
    ...
    def forward(self, x):
        ...
        spatial_output = decoded.view(batch_size, 1, self.output_height, self.output_width)
...
def add_coordinates_if_missing(df):
    ...
    print("\n🔍 일부 좌표 결측치에 대해 주소 검색(Geocoding)을 시도합니다...")
...
def create_static_grid(file_path, lat_bins, lon_bins, label="Data"):
    """개선된 정적 그리드 생성 (ConvLSTM 버전과 동일)"""
    print(f"\n[{label}] '{os.path.basename(file_path)}' 처리 중...")
    try:
...
def train_model_improved(model, train_loader, val_loader, num_epochs=100, learning_rate=0.0001, 
    ...
    print(f"\n🚀 학습 시작 (LSTM 3-Channel Model, Loss: {loss_type}, Early Stopping Patience: {patience})...")
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            save_path = os.path.join(SAVE_DIR, 'best_lstm_model.pth')
            torch.save({
                'epoch': epoch,
...
def main():
    ...
    print(f"🔷 LSTM 모델 (3채널: Parking + Camera + POI)")
    ...
    if os.path.exists(camera_file):
        ...
        print(f"⚠️ 카메라 파일 없음: {camera_file}")

    # 4. POI 데이터 로드 (Channel 3)
    poi_grid = None
    ...
    # 7. LSTM 모델 생성 및 학습
    print(f"\n모델 생성: LSTM (3-Channel, input_channels={input_ch})")
    model = LSTMModel(
    ...
    print(f"📊 모델 파라미터: {trainable_params:,} / {total_params:,}")
    ...
    # 8. 평가
    load_path = os.path.join(SAVE_DIR, 'best_lstm_model.pth')
    ...
    print(f"\n{'='*60}")
    print(f"최종 테스트 결과 (LSTM 3-Channel Model)")
    print(f"{'='*60}")
    ...
    plt.title('Training History (LSTM 3-Channel Model)', fontsize=14)
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
