# 해운대 전이학습 - Batch Norm 적용 버전 (3채널: 주정차, CCTV, POI)
import os
import glob
import random
import pandas as pd
...
# 설정
SEOUL_MODEL_PATH = './convlstm_improved_results/best_convlstm_model.pth'
SAVE_DIR = 'haeundae_transfer_bn_3ch_results'
...
def main():
    ...
    print("\n[2] 모델 초기화 및 전이학습 설정 (Batch Norm 적용)")
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('ConvLSTM Training History (with BatchNorm)')
    plt.legend()
    ...
    print(f"\n{'='*50}")
    print(f"최종 성능 평가 (ConvLSTM + BatchNorm)")
    print(f"{'='*50}")
