...
# [설정] 결과 저장 폴더 지정
# ---------------------------------------------------------
SAVE_DIR = 'convlstm_improved_results'
...
class ConvLSTM(nn.Module):
    def __init__(self, input_channels, hidden_channels, kernel_size, num_layers):
        ...
        for i in range(num_layers):
            ...
            bn_list.append(nn.BatchNorm2d(hidden_channels))
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        for layer_idx in range(self.num_layers):
            ...
            layer_output_flat = layer_output.view(b * s, ch, h_dim, w_dim)
            layer_output_flat = self.bn_list[layer_idx](layer_output_flat)
            layer_output = layer_output_flat.view(b, s, ch, h_dim, w_dim)
...
def train_model_improved(model, train_loader, val_loader, num_epochs=100, learning_rate=0.0001, 
    ...
    print(f"\n🚀 학습 시작 (Loss: {loss_type}, Early Stopping Patience: {patience})...")
...
def main():
    ...
    plt.title('Training History (Improved)', fontsize=14)
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
